...
@onready var anim_upper = $AnimationPlayerUpper
@onready var anim_lower = $AnimationPlayerLower
@onready var lock_on_area = $Area2D
...
# Attack buffer — stores a queued attack pressed during animation
...
func _do_toss() -> void:
	if current_weapon == null:
		return  # empty handed — does nothing
	if not current_weapon.can_toss:
		return
...
func _physics_process(delta: float) -> void:
	...
	# --- Priority 3: Main and Alt Attack ---
	if not is_dodging and not is_tossing:
...
func _process(_delta: float) -> void:
	...
	if is_dodging:
		$Body.rotation = dodge_direction.angle() - rotation
	else:
		$Body.rotation = 0.0

	# Upper body — idle/walk only when no action playing
